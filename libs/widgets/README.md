...
The `@one-inch-community/widgets` module is one of the key modules in the application. It serves as
the visual embodiment of the SDK, implementing large UI components that contain significant business
...
- **notification**: Although it doesn't contain a lot of business logic, this submodule is a crucial
  part of the application, aimed at enhancing transparency and user awareness of the application's
  operations.
...
- **token-icon**: A small submodule that handles the loading and caching of token icons for
  different networks, ensuring that token representations are accurate and readily available.

- **wallet-manager**: A set of visual components that implement the logic for connecting to Web3
  wallets, providing users with seamless wallet integration.

## Purpose

The `@one-inch-community/widgets` module plays a crucial role in bringing the business logic of the
SDK to life through its visual components. By integrating complex UI elements with underlying
business processes, it provides a cohesive and user-friendly interface that simplifies the
interactions with tokens, swaps, and Web3 wallets.
...
The `@one-inch-community/widgets` module is essential for delivering a robust and user-friendly
