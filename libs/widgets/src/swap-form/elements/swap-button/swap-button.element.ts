...
export class SwapButtonElement extends LitElement {
  ...
  @state() private buttonState: SwapButtonState =
    storage.get<SwapButtonState>('fusion_swap_button_state', JsonParser) ??
  ...
  private readonly exceedingMaximumAmount$ = combineLatest([
    ...
    merge(this.chainId$, this.block$).pipe(startWith(null)),
  ]).pipe(
  ...
  override connectedCallback() {
    ...
    subscribe(this, [
      ...
      this.sourceToken$.pipe(tap((token) => (this.srcToken = token))),
  ...
  protected override render() {
    ...
    return html`
      ...
      >
        ${when(
          this.buttonState === SwapButtonState.unsupportedChain,
          () => html`
        ...
        ${when(
          this.buttonState === SwapButtonState.updateSwapData,
          () => html` <span>${translate('widgets.swap-form.swap-button.update-swap-data')}</span>`
        )}
        ${when(
          this.buttonState === SwapButtonState.waitingApproveTransaction,
          () =>
            html` <span
              >${translate('widgets.swap-form.swap-button.waiting-approve-transaction')}</span
            >`
        )}
        ${when(
          this.buttonState === SwapButtonState.readyToSwap,
          () => html` <span>${translate('widgets.swap-form.swap-button.confirm-swap')}</span>`
        )}
        ${when(
        ...
        ${when(
          this.buttonState === SwapButtonState.checkAllowance,
          () => html` <span>${translate('widgets.swap-form.swap-button.check-allowance')}</span>`
        )}
        ${when(
          this.buttonState === SwapButtonState.wrapNativeToken,
          () =>
            html` <span
              >${translate('widgets.swap-form.swap-button.native-token-not-supported')}</span
            >`
        )}
        ${when(
          this.buttonState === SwapButtonState.lowAllowanceNeedApprove,
          () => html` <span>${translate('widgets.swap-form.swap-button.approve-and-swap')}</span>`
        )}
        ${when(
          this.buttonState === SwapButtonState.lowAllowanceNeedPermit,
          () => html` <span>${translate('widgets.swap-form.swap-button.permit-and-swap')}</span>`
        )}
        ${when(
          this.buttonState === SwapButtonState.walletNotConnected,
          () => html` <span>${translate('widgets.swap-form.swap-button.connect-wallet')}</span>`
        )}
        ${when(
          this.buttonState === SwapButtonState.unselectedSourceToken,
          () =>
            html` <span>${translate('widgets.swap-form.swap-button.select-source-token')}</span>`
        )}
        ${when(
          this.buttonState === SwapButtonState.unselectedDestinationToken,
          () =>
            html` <span
              >${translate('widgets.swap-form.swap-button.select-destination-token')}</span
            >`
        )}
        ${when(
          this.buttonState === SwapButtonState.zeroAmount,
          () =>
            html` <span>${translate('widgets.swap-form.swap-button.enter-amount-to-swap')}</span>`
        )}
        ${when(
          this.buttonState === SwapButtonState.rateNotExist,
          () =>
            html` <span>${translate('widgets.swap-form.swap-button.no-liquidity-for-swap')}</span>`
        )}
        ${when(
          this.buttonState === SwapButtonState.exceedingMaximumAmount,
          () => html`
            <span class="off-hover"
              >${translate('widgets.swap-form.swap-button.insufficient-balance', {
                symbol: this.srcToken?.symbol,
              })}</span
            >
            <span class="on-hover"
              >${translate('widgets.swap-form.swap-button.set-max', {
                symbol: this.srcToken?.symbol,
              })}</span
            >
            <br />
        ...
        ${when(
          ...
          () => html`
            <span class="off-hover"
              >${translate('widgets.swap-form.swap-button.wait-wallet-response')}</span
            >
            <span class="on-hover"
              >${translate('widgets.swap-form.swap-button.permit-swap-in-wallet')}</span
            >
            <br />
          `
        )}
        ${when(
          ...
          () => html`
            ...
            >
            <span class="on-hover"
              >${translate('widgets.swap-form.swap-button.allow-swap-in-wallet')}</span
            >
            <br />
          `
        )}
  ...
  private isNeedCheckAllowance() {
    ...
    return this.srcToken !== null && !isTokensEqual(this.srcToken, this.lastPermitCheckToken)
  }
}
