...
export class InputElement extends LitElement {
  ...
  private readonly loaderView$ = defer(() =>
    this.loading$.pipe(
      // map(() => true),
      map((state) => {
  ...
  override connectedCallback() {
    ...
    const loading$: Observable<void> = this.loading$.pipe(
      switchMap((state) => {
        ...
        if (!state && this.input.classList.contains('loading')) {
          return from(
            this.input.animate([{}, { opacity: 1 }], { duration: 2000 }).finished.then(() => {
    ...
    const updateInputValue$ = combineLatest([this.amount$, this.token$]).pipe(
      map(([amount, token]) => {
      ...
      <div class="input-container ${classMap(classes)}">
        <div class="flex-container symbol-title-name">
          <div class="input-title">
            ${choose(this.tokenType, [
              ['source', () => html`${translate('widgets.swap-form.input.title.you_pay')}`],
              [
                'destination',
                () => html`${translate('widgets.swap-form.input.title.you_receive')}`,
              ],
            ])}
          </div>
        ...
        <div class="flex-container balance-amount-fiat">
          <div class="balance-and-max">
            ...
            ${when(
              this.connectedAddress && this.token && this.tokenType === 'source',
              () => html`
                <inch-button
                  @click="${async () => {
                  ...
                  type="secondary"
                  size="xs"
                  loader="${ifDefined(this.setMaxInProgress ? '' : undefined)}"
                  >MAX</inch-button
                >
              `
            )}
  ...
  private tokenView(token: IToken) {
    ...
    return html`
      <button
        @click="${(event: MouseEvent) => this.openTokenSelector(event)}"
      ...
      >
        ...
        ${when(
          ...
          () => html`<inch-icon icon="chevronDown16"></inch-icon>`
        )}
      </button>
