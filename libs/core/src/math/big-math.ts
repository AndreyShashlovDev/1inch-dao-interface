export class BigMath {
  private constructor() {
    throw new Error(
      'BigMath does not support instance creation. Use static methods: div, mul, add, sub, avr, pow, min, max.'
    )
  }
  ...
  static avr(
    ...
    const scaledNumbers = numbers.map(
      (num) => (num * scale) / BigInt(10) ** BigInt(numbersDecimals)
    )
