import { QueueCache } from '@1inch-community/core/cache'
import {
  ChainId,
...
import { buildTokenId } from './token-id'
...
export class TokenSchema {
  ...
  async setTokens(tokens: ITokenV2Dto[]) {
    ...
    for (const token of tokens) {
      ...
      tableTokens.push({
        id: buildTokenId(chainId, token.address),
  ...
  async getSortedByPriorityAndBalanceTokenAddresses(
    ...
    await this.tokens
      ...
      .each((record) => {
        if (_filterPattern !== '') {
          if (
            !record.address.toLowerCase().includes(_filterPattern) &&
            !record.name.toLowerCase().startsWith(_filterPattern) &&
            !record.symbol.toLowerCase().startsWith(_filterPattern)
          )
            return
        }
