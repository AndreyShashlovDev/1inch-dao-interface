...
import { getChainIdsFromStorage, setActiveAddress } from './storage'

export class GlobalDataAdapter
  implements IDataAdapter, IGlobalDataAdapterInternal, IGlobalDataAdapter
{
  private readonly context = lazyAppContext('GlobalDataAdapter')

  private readonly chainIdInner$ = lazy(
    () =>
      new BehaviorSubject<ChainId[]>(
        getChainIdsFromStorage(this.context.value.storage) ?? [ChainId.eth]
      )
  )
  ...
  //   tap(chainId => chainId && setChainIdInStorage(this.context.value.storage, chainId)),
  //   shareReplay({ bufferSize: 1, refCount: true })
  // );

  // todo: stub
  readonly chainId$ = of(ChainId.eth)
  ...
  async getChainId(): Promise<ChainId> {
    if (!this.dataProvider.currentActiveAdapter) {
      return this.chainIdInner$.value.value[0]
    }
    const data = await this.getProviderDataAdapter()
    if (!data) return this.chainIdInner$.value.value[0]
    const chainId = await data.getChainId()
    if (!chainId) return this.chainIdInner$.value.value[0]
    return chainId
  }
  ...
  setChainIds(chainId: ChainId[]): void {
    this.chainIdInner$.value.next(chainId)
  }
